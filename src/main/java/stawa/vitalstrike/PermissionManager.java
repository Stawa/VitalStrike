...
/**
 * Manages permissions for VitalStrike plugin, particularly for group-based damage formats. Handles
 * loading, checking, and retrieving group permissions from configuration.
 */
public class PermissionManager {
  ...
  /**
   * Loads group-based damage formats from configuration. Sets up permission-based damage format
   * groups.
   *
  ...
  /**
   * Loads format settings for a specific group. Processes permission and damage formats for a
   * single group.
   *
  ...
  /**
   * Loads damage formats for a specific group. Maps damage types to their display formats for the
   * group.
   *
   ...
   * @param defaultFormat the default format to use if no permission-based format is found
   * @return the formatted damage string
   */
  public String getDamageFormat(Player player, String damageType, String defaultFormat) {
    String damageIndicatorType =
        plugin.getConfig().getString("damage-indicator", "simple-damage-formats");
  ...
  private String getGroupBasedDamageFormat(Player player, String damageType, String defaultFormat) {
    ConfigurationSection groupsSection =
        plugin.getConfig().getConfigurationSection("group-damage-formats");
   ...
   */
  private String findGroupFormat(
      Player player, String damageType, ConfigurationSection groupsSection) {
  ...
  private String getFormatForGroup(Player player, String groupKey, String damageType) {
    ...
    String format =
        plugin
            .getConfig()
            .getString("group-damage-formats." + groupKey + ".damage-formats." + damageType);
    if (format != null) {
      return format;
    }
    ...
    return plugin
        .getConfig()
        .getString("group-damage-formats." + groupKey + ".damage-formats.default");
  }
   ...
   *
   * @param damageType the type of damage
   * @param defaultFormat the default format to use if no default group format is found
   * @return the formatted damage string
   */
  private String getDefaultGroupFormat(String damageType, String defaultFormat) {
    boolean useSimpleFormats =
        plugin.getConfig().getBoolean("group-damage-formats.default.use-simple-formats", false);
    if (useSimpleFormats) {
      return defaultFormat;
    }

    String format =
        plugin.getConfig().getString("group-damage-formats.default.damage-formats." + damageType);
    if (format != null) {
      return format;
    }
  ...
  /**
   * Checks if a player has an explicitly assigned permission (not inherited from OP).
   *
   * @param player the player to check
