...
/**
 * Utility class for WorldGuard integration in VitalStrike. Handles PvP region checks and
 * WorldGuard-specific functionality.
 */
public class WorldGuardUtil {
  ...
  /**
   * Initializes the WorldGuard integration. Checks if WorldGuard is present and enables
  ...
  /**
   * Checks if PvP is allowed at a player's location. Takes into account WorldGuard regions and
  ...
  public static boolean isPvPAllowed(Player player) {
    if (!worldGuardEnabled
        || !plugin.getConfig().getBoolean("world-settings.respect-worldguard-pvp", true)) {
      return true;
    }
    ...
    try {
      ...
      com.sk89q.worldguard.protection.ApplicableRegionSet set =
          query.getApplicableRegions(com.sk89q.worldedit.bukkit.BukkitAdapter.adapt(loc));
