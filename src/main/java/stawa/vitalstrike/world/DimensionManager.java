...
public class DimensionManager {
  ...
  /**
   * Loads dimension settings from the plugin configuration. Initializes enabled states and damage
   * multipliers for each dimension.
   */
  private void loadDimensionSettings() {
    ConfigurationSection dimensions =
        plugin.getConfig().getConfigurationSection("world-settings.dimensions");
    if (dimensions == null) {
      return;
    }
   ...
   * @return true if damage indicators are enabled in the dimension, false otherwise
   */
  ...
  /**
   * Reloads dimension settings from the configuration. Clears existing settings and reloads from
   * config.
   */
  public void reload() {
